...
- **screens/** — Auth, Onboarding, Home, Insights, Settings, AddEditHabit, HabitDetail, ChallengeReward (8 screens, surface-area budget). Home renders the auto coaching nudge; Insights (charts tab) renders the manual weekly/monthly AI reflection.
- **navigation/** — `index.tsx` RootNavigator gates on auth then onboarding: `!session ? Auth : !onboarded ? Onboarding : Tabs(Home/Insights/Settings)+modals`; notification-tap routing survives cold start; flushes pending sync on onboard.
- **supabase/** — `migrations/*.sql` (per-user normalized schema: profiles/settings/habits/challenges/daily_logs + `coach_messages` AI cache), `functions/ai-coach/` (Deno Edge Function → Claude API), `config.toml`. Project ref lives in `.supabase-create.txt` (gitignored).
